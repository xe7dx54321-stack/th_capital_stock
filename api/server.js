...
const DB_PATH = path.resolve(__dirname, "..", "01_data", "db", "smr.db");
...
app.get("/api/dashboard", (_req, res) => {
  try {
    const data = cached("dashboard", () => {
      const db = getDb();
      const poolTotal = db.prepare("SELECT COUNT(DISTINCT ts_code) as cnt FROM stock_pool_current").get();
      const poolByType = db.prepare("SELECT pool_type as type, COUNT(*) as count FROM stock_pool_current GROUP BY pool_type ORDER BY count DESC").all();
      const ahCoverage = db.prepare("SELECT COUNT(DISTINCT ts_code) as cnt FROM daily_bar").get();
      const usCoverage = db.prepare("SELECT COUNT(DISTINCT symbol) as cnt FROM us_daily_bar").get();
      const withFundamentals = db.prepare("SELECT COUNT(DISTINCT ts_code) as cnt FROM factor_daily").get();
      const newsCount = db.prepare("SELECT COUNT(*) as cnt FROM news_items").get();
      const riskAlerts = db.prepare("SELECT COUNT(*) as cnt FROM risk_alert").get();
      db.close();
      return {
        summary: {
          poolTotal: poolTotal?.cnt || 0,
          ahCoverage: ahCoverage?.cnt || 0,
          usCoverage: usCoverage?.cnt || 0,
          withFundamentals: withFundamentals?.cnt || 0,
          newsCount: newsCount?.cnt || 0,
          riskAlerts: riskAlerts?.cnt || 0,
        },
        poolByType: poolByType.length > 0 ? poolByType : [],
        dataFreshness: [
...
app.get("/api/discoveries", (_req, res) => {
  try {
    const data = cached("discoveries", () => {
      const db = getDb();

      // 步骤1：从 stock_pool_current 读取真实的行业映射（权威数据源）
      // 这是你真正关注的5大方向，每个标的在 stock_pool_current.sector 中
      const poolRows = db.prepare(
        "SELECT DISTINCT ts_code, sector FROM stock_pool_current"
      ).all();
      const coveredCodes = new Set(poolRows.map(r => r.ts_code));
      const codeToSector = new Map();
      for (const r of poolRows) {
        codeToSector.set(r.ts_code, r.sector);
      }
      ...
      const focusSectors = new Set(Object.keys(sectorCN));

      // 步骤3：硬编码映射（只包含 stock_pool_current 中实际存在的 57 只股票）
      // ⚠️ 请不要随意添加不在池子里的代码（如 TSM/ASML/00700.HK 等）
      // 如果发现管线新增股票，请先确认它已进入 stock_pool_current
      const knownSectorMap = {
        // --- semiconductor_compute：算力/AI芯片公司 ---
        "NVDA": "semiconductor_compute",     // 英伟达（GPU）
        "AMD": "semiconductor_compute",      // 超威半导体（CPU/GPU）
        "AVGO": "semiconductor_compute",     // 博通（AI加速器）
        "INTC": "semiconductor_compute",     // 英特尔（CPU）
        "MU": "semiconductor_compute",       // 美光（存储）
        "SNPS": "semiconductor_compute",     // Synopsys（EDA）
        "CDNS": "semiconductor_compute",     // Cadence（EDA）
        "603986.SH": "semiconductor_compute", // 兆易创新
        "688008.SH": "semiconductor_compute", // 澜起科技
        "688041.SH": "semiconductor_compute", // 海光信息
        "688256.SH": "semiconductor_compute", // 寒武纪
        "688521.SH": "semiconductor_compute", // 芯原股份
        "688525.SH": "semiconductor_compute", // 安集科技
        "688800.SH": "semiconductor_compute", // 瑞芯微
        "300593.SZ": "semiconductor_compute", // 新雷能
        "301269.SZ": "semiconductor_compute", // 华大九天
        "00981.HK": "semiconductor_compute",  // 中芯国际
        "01347.HK": "semiconductor_compute",  // 华虹半导体

        // --- semiconductor_photonics：光模块/光子 ---
        "LITE": "semiconductor_photonics",   // Lumentum
        "COHR": "semiconductor_photonics",   // Coherent
        "MRVL": "semiconductor_photonics",   // Marvell
        "VRT": "semiconductor_photonics",    // Vertiv
        "000063.SZ": "semiconductor_photonics", // 中兴通讯
        "002281.SZ": "semiconductor_photonics", // 光迅科技
        "002796.SZ": "semiconductor_photonics", // 世嘉科技
        "002837.SZ": "semiconductor_photonics", // 英维克
        "300308.SZ": "semiconductor_photonics", // 中际旭创
        "300394.SZ": "semiconductor_photonics", // 天孚通信
        "300502.SZ": "semiconductor_photonics", // 新易盛
        "300620.SZ": "semiconductor_photonics", // 光库科技
        "688205.SH": "semiconductor_photonics", // 德科立

        // --- embodied_ai：具身智能/机器人 ---
        "TSLA": "embodied_ai",               // 特斯拉（机器人/自动驾驶）
        "002050.SZ": "embodied_ai",          // 三花智控
        "002600.SZ": "embodied_ai",          // 领益智造
        "002957.SZ": "embodied_ai",          // 朗特智能
        "300124.SZ": "embodied_ai",          // 汇川技术
        "301368.SZ": "embodied_ai",          // 丰立智能
        "600580.SH": "embodied_ai",          // 卧龙电驱
        "601689.SH": "embodied_ai",          // 拓普集团
        "603728.SH": "embodied_ai",          // 鸣志电器
        "688017.SH": "embodied_ai",          // 绿的谐波
        "688322.SH": "embodied_ai",          // 奥比中光
        "09980.HK": "embodied_ai",           // 东鹏控股
        "872808.BJ": "embodied_ai",          // 五新隧装

        // --- ai_agent：AI智能体/平台/大模型公司 ---
        "MSFT": "ai_agent",                  // 微软
        "CRM": "ai_agent",                   // Salesforce
        "NOW": "ai_agent",                   // ServiceNow
        "002230.SZ": "ai_agent",             // 科大讯飞
        "301171.SZ": "ai_agent",             // 易点天下
        "688111.SH": "ai_agent",             // 金山办公
        "603039.SH": "ai_agent",             // 泛微网络
        "00020.HK": "ai_agent",              // 商汤-W
        "09988.HK": "ai_agent",              // 阿里巴巴-W

        // --- quantum：量子科技 ---
        "IONQ": "quantum",
        "QBTS": "quantum",
        "RGTI": "quantum",
        "688027.SH": "quantum",              // 国盾量子
      };

      // 步骤4：从 news_items 提取被提及但尚未在关注池中的股票
      ...
      for (const row of newsRows) {
        try {
          ...
          for (const t of tickers) {
            if (!t || coveredCodes.has(t)) continue; // 跳过已在池中的
            tickerCounter.set(t, (tickerCounter.get(t) || 0) + 1);
            if (!tickerNews.has(t)) {
              tickerNews.set(t, { title: row.title, at: row.published_at });
            }
          }
        } catch {}
      }

      // 步骤5：检测价格异动（从 daily_bar 和 us_daily_bar 找涨幅超 5% 的股票）
      const priceSpikes = new Map(); // ticker -> {pct_chg, trade_date}
      // A股/港股
      const ahSpikes = db.prepare(
      ...
      for (const r of ahSpikes) {
        if (!coveredCodes.has(r.ts_code)) {
          priceSpikes.set(r.ts_code, { pct_chg: r.pct_chg, trade_date: r.trade_date });
        }
      }
      // 美股
      const usSpikes = db.prepare(
        `SELECT symbol, pct_chg, trade_date FROM us_daily_bar
         WHERE trade_date = (SELECT MAX(trade_date) FROM us_daily_bar)
           AND pct_chg IS NOT NULL AND pct_chg >= 5
         ORDER BY pct_chg DESC LIMIT 20`
      ).all();
      for (const r of usSpikes) {
        if (!coveredCodes.has(r.symbol)) {
          priceSpikes.set(r.symbol, { pct_chg: r.pct_chg, trade_date: r.trade_date });
        }
      }
      ...
      for (const [ticker, spike] of sortedSpikes.slice(0, 10)) {
        let market = "其他";
        if (ticker.endsWith(".SH") || ticker.endsWith(".SZ")) market = "A";
        else if (ticker.endsWith(".HK")) market = "H";
        else if (/^[A-Z]/.test(ticker)) market = "US";

        let sector = null;
        if (codeToSector.has(ticker)) sector = codeToSector.get(ticker);
        else if (knownSectorMap[ticker]) sector = knownSectorMap[ticker];

        const sectorName = sector ? (sectorCN[sector] || sector) : "待评估";
        const isInFocus = sector ? focusSectors.has(sector) : false;
        const name = getStockName(ticker) || ticker;
        const priority = spike.pct_chg >= 8 ? "high" : "medium";
      ...
      // 6.2：加入新闻提及候选
      const sorted = Array.from(tickerCounter.entries()).sort((a, b) => b[1] - a[1]);

      for (const [ticker, mentions] of sorted.slice(0, 15)) {
        let market = "其他";
        if (ticker.endsWith(".SH") || ticker.endsWith(".SZ")) market = "A";
        else if (ticker.endsWith(".HK")) market = "H";
        else if (/^[A-Z]/.test(ticker)) market = "US";
        ...
        // 优先级：新闻提及越多越高
        let priority = "low";
        if (mentions >= 20) priority = "high";
        else if (mentions >= 8) priority = "medium";

        const news = tickerNews.get(ticker);
        const name = getStockName(ticker) || ticker;

        // 关键修复：判断行业
        // 先查 stock_pool_current（理论上已过滤掉了，这里做双层保险）
        // 再查 knownSectorMap（对于未入池但知名的股票）
        // 都查不到就标记"待评估"
        let sector = null;
        if (codeToSector.has(ticker)) {
          sector = codeToSector.get(ticker);
        } else if (knownSectorMap[ticker]) {
          sector = knownSectorMap[ticker];
        }
...
// ============================================================
// API: GET /api/news（列表：含正文摘要，可点击查看详情）
// API: GET /api/news/:id（单条新闻详情：含完整正文 + 解读）
// ============================================================
app.get("/api/news", (_req, res) => {
  try {
    const data = cached("news", () => {
      const db = getDb();
      ...
      const processed = items.map((r) => {
        ...
        if (r.tickers_json) {
          try {
            const parsed = JSON.parse(r.tickers_json);
            if (Array.isArray(parsed)) tickers = parsed.filter((t) => t && typeof t === "string");
          } catch {}
        }
        const body = r.body || "";
        return {
          ...
          title: r.title,
          source: r.source_key,
          sourceName: r.source_name || r.source_key,
      ...
      const sources = Array.from(new Set(items.map((i) => i.source_key))).sort();
      return { items: processed, sources, updatedAt: new Date().toISOString() };
    });
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get("/api/news/:id", (req, res) => {
  try {
    ...
    const db = getDb();
    const r = db.prepare(
      `SELECT id, title, body, source_key, source_name, published_at, tickers_json, url, credibility, themes_json
    ...
    try { tickers = JSON.parse(r.tickers_json || "[]").filter((t) => t && typeof t === "string"); } catch {}
    let themes = [];
    try { themes = JSON.parse(r.themes_json || "[]"); } catch {}

    // ===== 智能解读：基于正文/标题/来源给一个简短的中文解读 =====
    const body = (r.body || "") + " " + (r.title || "");
    const bodyLower = body.toLowerCase();
    const insights = [];
    if (/\b(?:增持|买入|看好|优于|推荐|outperform|buy|strong buy)\b/i.test(body)) {
      insights.push({ type: "bull", text: "文中出现买入/增持/看好等正面评级，属于利好信号。" });
    }
    if (/\b(?:减持|卖出|中性|回避|underperform|sell|hold)\b/i.test(body)) {
      insights.push({ type: "bear", text: "文中出现减持/卖出/中性等负面或中性评级，注意风险。" });
    }
    if (/\b(?:业绩|营收|净利润|盈利|亏损|同比|增长|下降)\b/.test(body)) {
      insights.push({ type: "fundamentals", text: "内容涉及业绩/营收/盈利情况，属于基本面驱动事件。" });
    }
    if (/\b(?:AI|人工智能|大模型|算力|GPU|芯片|半导体|具身|机器人)\b/i.test(body)) {
      insights.push({ type: "theme", text: "内容涉及 AI/算力/半导体/机器人等科技主题方向。" });
    }
    if (/\b(?:量子|量子计算|量子通信)\b/.test(body)) {
      insights.push({ type: "theme", text: "内容涉及量子科技主题方向。" });
    }
    if (/\b(?:订单|中标|合同|客户|订单量)\b/.test(body)) {
      insights.push({ type: "order", text: "内容涉及订单/中标/合同等商业落地信号。" });
    }
    if (/\b(?:监管|处罚|警告|风险|合规|诉讼|调查|调查)\b/.test(body)) {
    ...
    res.json({
      ...
      url: r.url,
      credibility: r.credibility || "",
      credibilityText: credibilityMap[r.credibility] || "未标注",
      insights,
      updatedAt: new Date().toISOString(),
    });
  } catch (err) {
