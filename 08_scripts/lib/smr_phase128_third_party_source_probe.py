...
def probe_third_party_sources(skip_network=False):
    targets=plan_probe_targets()["phase128_probe_target_planner"]["targets"]
    tp=[t for t in targets if t.get("category")=="third_party"]
    results=[]
    for t in tp:
        ...
        elif t["source_id"]=="akshare_hk":
            r["probe_status"]="available"
            r["reachable"]=True
            r["note"]="library_already_used_in_phase83_hk_adapter"
        else:
    ...
    return {"phase128_third_party_source_probe":{"total":len(results),"available":available,"blocked":blocked,"skipped":skipped,"results":results,"mock_used":False,"fixture_used":False,"raw_saved":False}}
