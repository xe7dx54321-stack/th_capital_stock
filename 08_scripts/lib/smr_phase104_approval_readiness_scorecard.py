...
def build_approval_readiness_scorecard():
    reg=build_approval_domain_registry()
    domains=reg["phase104_approval_domain_registry"]["domains"]
    status_count={}
    for d in domains:
    ...
    scorecard={
        ...
        "critical_blockers":["operator_identity_not_ready"],
        "readiness_pct":round(status_count.get("ready",0)/len(domains)*100,1),
        "mock_used":False,
        "fixture_used":False
    }
    return {"phase104_approval_readiness_scorecard":scorecard}
