...
def build_quality_gate(validator, state_preview):
    ...
    if v["status"] == "no_input":
        violations = 0
        status = "pass"
        qualifier = "no_input_pending"
    else:
        violations = 0 if v["valid_entries"]>0 else 1
        status = "pass" if violations==0 else "fail"
        qualifier = "input_present" if v["valid_entries"]>0 else "input_failed_validation"
    checks = {"input_read_attempted":True,"validator_status":v["status"],"state_preview_generated":s["entries"]>0,"state_not_updated":s["state_not_updated"],"no_auto_activation":True,"no_target_price":True}
    return {"phase170_quality_gate":{"status":status,"violations":violations,"qualifier":qualifier,"checks":checks,"mock_used":False,"fixture_used":False}}
def build_cannot_conclude_guard():
    reserved = ["300394 CNINFO org_id missing","300394 thesis unconfirmed","688041 derived valuation label only","validated_input != activation_executed","formal_state_preview != state_update","tier_proposal != tier_assignment","agent_task_delta != task_execution","daily_monitoring_preview != monitoring_update","owner_input_read != owner_approval","manifest != action_list","no_input_is_not_system_failure","watch_core_updated=false","candidate_auto_activated=false","tier_update_executed=false","target_price_output_allowed=false","position_sizing_allowed=false","broker_integration_allowed=false"]
