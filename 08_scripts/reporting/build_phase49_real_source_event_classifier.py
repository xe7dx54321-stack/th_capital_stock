...
def build(conn,ticker):
    ...
    return build_classifier_result(sources,ticker)
def md(p):
    c=p.get("real_source_event_classifier")or{}
    l=[f"# Phase 49 Real Source Event Classifier: {p.get('ticker')}","",f"- sources_checked: {c.get('sources_checked')}",f"- events_classified: {c.get('events_classified')}","","## Events"]
    for r in c.get("event_rows")or[]: l.append(f"- {r.get('event_type')}: {r.get('source_title')}")
    return "\n".join(l).rstrip()+"\n"
def main():
