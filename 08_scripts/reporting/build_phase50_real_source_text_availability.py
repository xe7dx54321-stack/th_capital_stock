...
from smr_real_source_text_availability import build_report
from smr_research_review_lifecycle import TARGET_REVIEW_TICKER
if hasattr(sys.stdout,"reconfigure"): sys.stdout.reconfigure(encoding="utf-8")
def build(conn,ticker): return build_report(get_sample_sources(ticker),ticker)
def md(p): a=p.get("real_source_text_availability")or{}; return f"# Phase 50 Text Availability: {p.get('ticker')}\n\n- sources_checked: {a.get('sources_checked')}\n- text_available: {a.get('text_available')}\n"
def main():
