#!/usr/bin/env python3
"""Phase 67 searchkey inventory report."""
import argparse,json,sys
from pathlib import Path
L=Path(__file__).resolve().parents[1]/"lib"
if str(L) not in sys.path: sys.path.insert(0,str(L))
if hasattr(sys.stdout,"reconfigure"): sys.stdout.reconfigure(encoding="utf-8")
from smr_cninfo_searchkey_query_engine import query_by_searchkeys as build
def _md(r):
    inv=r.get("cninfo_searchkey_inventory",r)
    lines=["# CNINFO Searchkey Inventory",""]
    lines.append("Queries run: "+str(inv.get("searchkey_queries_run",0)))
    ...
    if inv.get("keyword_group_breakdown"):
        lines.append("");lines.append("Group breakdown:")
        for k,v in inv["keyword_group_breakdown"].items(): lines.append("- "+k+": "+str(v))
    return "\n".join(lines)
def main():
