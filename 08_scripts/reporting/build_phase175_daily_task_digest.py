# Phase175 daily and weekly task digest
import json, sys, os
sys.path.insert(0,os.path.join(os.path.dirname(__file__),"..","lib"))
from smr_phase175_task_executor import run_all_tasks
from datetime import datetime

def build_daily_task_digest(mode="execute"):
    r = run_all_tasks(mode)
    ex = r["phase175_task_executor"]
    return {"phase175_daily_task_digest":{"date":datetime.now().strftime("%Y-%m-%d"),"total_tasks":ex["total_tasks"],"completed":ex["completed"],"failed":ex["failed"],"deferred":ex["deferred"],"summary":f"Daily research task run: {ex['completed']} completed, {ex['failed']} failed, {ex['deferred']} deferred.","research_only":True,"no_trade_content":True,"mock_used":False,"fixture_used":False}}
...
if __name__=="__main__":
    ...
    digest_type = "daily" if "daily" in sys.argv[0] else "weekly"
    if digest_type == "daily":
        result = build_daily_task_digest(mode)
    else:
        result = build_weekly_task_digest(mode)
    if args.markdown:
        d = result[list(result.keys())[0]]
        print(f"# {'Daily' if digest_type=='daily' else 'Weekly'} Research Task Digest")
