...
def build(conn,ticker):
    ...
    ts=sum(1 for r in rows if r["quality_status_after"]=="passed_tracking_support")
    rr=sum(1 for r in rows if r["quality_status_after"]=="review_required")
    return build_decision(ticker,ts,rr)
