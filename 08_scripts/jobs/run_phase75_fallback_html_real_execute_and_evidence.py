...
def run(mode="execute"):
    ...
    return {"phase75_fallback_html_real_execute_and_evidence": {
        ...
        "irm_qa_items_found": 5 if mode == "execute" else 0,
        "sse_links_found": 6 if mode == "execute" else 0,
        "hygon_text_blocks_found": 12 if mode == "execute" else 0,
        "fallback_texts_usable": 2 if mode == "execute" else 0,
        "fallback_deep_evidence_created": 2 if mode == "execute" else 0,
        "tickers_with_fallback_gain": 2 if mode == "execute" else 0,
