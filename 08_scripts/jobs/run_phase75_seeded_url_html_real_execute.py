...
def fetch_and_extract(url, source_type):
    if not url:
        return {"url": "", "http_status": 0, "text_length": 0, "text_hash": "", "error": "empty_url"}
    try:
        ...
        return {"url": url, "source_type": source_type, "http_status": 200, "text_length": len(clean),
                "text_hash": text_hash(clean), "chinese_ratio": cr, "links_found": len(links),
                "quality_hint": qh, "text_preview": clean[:2000], "error": None}
...
def run(mode="execute", tickers=None):
    ...
    for t in tickers:
        ...
        for k in ["official_site", "ir_page"]:
            if ir.get(k):
                urls.append((ir[k], f"company_{k}", t))
        for ku in seed_known_urls(t):
            if ku.get("url"):
                urls.append((ku["url"], ku.get("source_type", "known_url"), t))
    if mode == "dry_run":
        rows = [{"url": u, "source_type": st, "ticker": tk, "http_status": 0, "text_length": 0, "quality_hint": "dry_run"} for u, st, tk in urls]
        return {"phase75_seeded_url_html_real_execute": {"mode": mode, "network_attempted": False,
            "seeded_urls_checked": len(urls), "html_pages_fetched": 0, "texts_extracted": 0, "texts_usable": 0,
            "rows": rows, "raw_saved": False, "ocr_used": False, "mock_used": False, "fixture_used": False}}
    rows = []
    for url, st, tk in urls:
        r = fetch_and_extract(url, st)
        r["ticker"] = tk
        rows.append(r)
    ok = sum(1 for r in rows if r.get("http_status") == 200)
    usable = sum(1 for r in rows if "usable" in str(r.get("quality_hint", "")))
    return {"phase75_seeded_url_html_real_execute": {"mode": mode, "network_attempted": network_attempted,
        "seeded_urls_checked": len(urls), "html_pages_fetched": ok, "texts_extracted": ok, "texts_usable": usable,
        "rows": rows, "raw_saved": False, "ocr_used": False, "mock_used": False, "fixture_used": False}}
