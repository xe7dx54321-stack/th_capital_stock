...
from build_phase148_candidate_dashboard import build

def run(mode="dry-run"):
    s = datetime.now().isoformat()
    r = build(); d = r["phase148_candidate_dashboard"]
    return {"phase148_candidate_pipeline": {
        "mode": mode, "started_at": s, "finished_at": datetime.now().isoformat(),
        "candidates": d["profiles"]["candidates"],
        "activation_plans": d["activation_plans"]["plans"],
