...
def build(conn,ticker,mode="dry-run"):
    ...
    return build_candidates(extractions,ticker,mode=mode)
def main():
    p=argparse.ArgumentParser(); p.add_argument("--db-path",default=str(DB_PATH)); p.add_argument("--ticker",default=TARGET_REVIEW_TICKER); p.add_argument("--dry-run",action="store_true"); p.add_argument("--execute",action="store_true"); p.add_argument("--json",action="store_true"); a=p.parse_args()
    mode="execute" if a.execute else "dry-run"; conn=sqlite3.connect(a.db_path)
    try: pl=build(conn,a.ticker,mode=mode)
    finally: conn.close()
