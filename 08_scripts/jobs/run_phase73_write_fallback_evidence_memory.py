...
def run(mode="dry_run",tickers=None):
 ...
 try:
  ...
  ev=build();items=ev.get("phase73_fallback_evidence_rerun",{}).get("rows",[])
 except:
  items=[]
 rows=[]
 for t in tickers:
 ...
 return {"phase73_fallback_evidence_memory_write":{"mode":mode,"records_written_total":total,"rows":rows,"memory_path_ignored":True,"mock_used":False,"fixture_used":False}}
def main():
