...
def build_memory(dry_run=False):
    ...
    recs=[]
    for row in rows:
        recs.append({"ticker":row["ticker"],"market":row["market"],"signal_id":row.get("signal_id",""),"freshness":row.get("freshness",""),"novelty":row.get("novelty",""),"source_type":"phase88_daily","dry_run":dry_run})
    return {"phase88_daily_external_evidence_memory":{"records_written":len(recs) if not dry_run else 0,"records_dry_run":len(recs),"rows":recs,"memory_path_ignored":True,"mock_used":False,"fixture_used":False}}
