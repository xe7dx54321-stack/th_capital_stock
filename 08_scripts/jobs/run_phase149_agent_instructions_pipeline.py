...
from build_phase149_agent_instructions_dashboard import build

def run(mode="dry-run"):
    s = datetime.now().isoformat()
    r = build(); d = r["phase149_agent_instructions_dashboard"]
    return {"phase149_agent_instructions_pipeline": {
        "mode": mode, "started_at": s, "finished_at": datetime.now().isoformat(),
        "agents_documented": d["instructions"]["agents"],
        "version": d["instructions"]["version"],
