...
class TestWatchlistUpdate(unittest.TestCase):
    def test_no_pending_order_trade(self):
        try:
            from build_phase66_watchlist_update_from_deep_disclosure import build
            r=build("300308.SZ")
            wu=r.get("watchlist_update_from_deep_disclosure",{})
    ...
    def test_has_decision(self):
        try:
            ...
            r=build("300308.SZ")
            wu=r.get("watchlist_update_from_deep_disclosure",{})
            self.assertIn("watchlist_decision",wu)
            self.assertTrue(wu["watchlist_decision"])
        except ImportError:
            self.skipTest("module not importable")
