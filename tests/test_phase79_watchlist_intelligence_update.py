...
class TestWatchlist(unittest.TestCase):
    def test_build(self):
        from build_phase79_watchlist_intelligence_update import build
        r=build();w=r["phase79_watchlist_intelligence_update"]
        self.assertGreater(w["updated_tickers"],0)
    def test_no_pending_order(self):
        from build_phase79_watchlist_intelligence_update import build
        r=build();rows=r["phase79_watchlist_intelligence_update"]["rows"]
        for row in rows:
