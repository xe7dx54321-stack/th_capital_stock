...
class TestEvidenceMemory(unittest.TestCase):
 def test_has_rows(self):
  from run_phase74_write_fallback_evidence_memory import run
  r=run("dry_run");m=r["phase74_fallback_evidence_memory_write"]
 ...
 def test_no_fake_write(self):
  ...
  r=run("dry_run");m=r["phase74_fallback_evidence_memory_write"]
  for row in m["rows"]:
   if row.get("reason","").startswith("no_fallback"):self.assertEqual(row["records_written"],0)
