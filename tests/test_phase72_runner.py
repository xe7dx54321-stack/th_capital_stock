...
class TestPhase72Runner(unittest.TestCase):
    def test_dry_run(self):
        from run_phase72_fallback_real_text_acquisition import run
        r = run(mode="dry_run"); p = r["phase72_fallback_real_text_acquisition"]
        self.assertGreater(len(p.get("steps", [])), 0)
    def test_execute(self):
        from run_phase72_fallback_real_text_acquisition import run
        r = run(mode="execute"); p = r["phase72_fallback_real_text_acquisition"]
        self.assertGreater(len(p.get("steps", [])), 0)
    def test_skip_network(self):
        from run_phase72_fallback_real_text_acquisition import run
        r = run(mode="execute", skip_network=True); p = r["phase72_fallback_real_text_acquisition"]
        self.assertGreater(len(p.get("steps", [])), 0)
