...
class TestResearchPacket(unittest.TestCase):
    def test_no_trade_advice(self):
        from build_phase72_research_packet import build
        r = build(); pkt = r["phase72_research_packet"]
        self.assertEqual(pkt.get("pending_created", -1), 0); self.assertEqual(pkt.get("paper_order_created", -1), 0); self.assertEqual(pkt.get("real_trade_created", -1), 0)
    def test_has_fallback(self):
        from build_phase72_research_packet import build
        r = build(); pkt = r["phase72_research_packet"]
        for t in pkt["tickers"]: self.assertIn("fallback", t)
